...
<svelte:head><title>{item.name} - Cuentas</title></svelte:head>
<div class="mb-4"><a href="/account/accounts" class="link link-neutral text-sm">&larr; Volver</a></div>

{#if editing}
<div class="card bg-base-200 p-6 max-w-xl">
  <div class="flex justify-between items-center mb-4"><h2 class="text-lg font-semibold">Editar Cuenta</h2><button class="btn btn-ghost btn-xs" onclick={() => editing = false}>✕</button></div>
  <form method="POST" action="?/update" use:enhance={handleSubmit}>
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      <div><label class="label" for="website"><span class="label-text">Sitio Web</span></label><input id="website" name="website" type="text" class="input input-bordered w-full" value={form?.website ?? item.website ?? ""} /></div>
      <div><label class="label" for="industry"><span class="label-text">Industria</span></label><input id="industry" name="industry" type="text" class="input input-bordered w-full" value={form?.industry ?? item.industry ?? ""} /></div>
      <div><label class="label" for="status"><span class="label-text">Estado</span></label><select id="status" name="status" class="select select-bordered w-full"><option value="active" selected={item.status === "active"}>Activo</option><option value="inactive" selected={item.status === "inactive"}>Inactivo</option></select></div>
...
<div class="card bg-base-200 p-6 max-w-xl">
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
    ...
    <div><span class="text-base-content/60 block text-xs uppercase tracking-wide">Sitio Web</span><span>{item.website ?? "—"}</span></div>
