...
<svelte:head><title>Pedidos</title></svelte:head>

<div class="flex justify-between items-center mb-4 p-3 border border-gray-300"><h1 class="text-2xl font-bold">Pedidos</h1><button class="btn btn-primary btn-sm" onclick={openNew}>+ Nuevo Pedido</button></div>

{#if showForm}
<div class="card bg-base-200 p-6 mb-6 max-w-xl">
  <div class="flex justify-between items-center mb-4"><h2 class="text-lg font-semibold">Nuevo Pedido</h2><button class="btn btn-ghost btn-xs" onclick={closeForm}>✕</button></div>
  <form method="POST" action="?/create" use:enhance={handleSubmit}>
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      <div><label class="label" for="quote_id"><span class="label-text">Quote ID</span></label><input id="quote_id" name="quote_id" type="text" class="input input-bordered w-full" value={form?.quote_id ?? ""} /></div>
      ...
      <div><label class="label" for="order_date"><span class="label-text">Fecha del Pedido</span></label><input id="order_date" name="order_date" type="date" class="input input-bordered w-full" value={form?.order_date ?? ""} /></div>
...
<div class="overflow-x-auto border border-gray-300">
  {#if data.items.length === 0}<div class="alert bg-base-200 max-w-md mt-4"><span>Aún no hay pedidos.</span></div>
  {:else}
  <table class="table">
    <thead><tr><th>Nombre</th><th class="hidden sm:table-cell">Total</th><th class="hidden md:table-cell">Estado</th><th>Fecha del Pedido</th><th></th></tr></thead>
    <tbody>{#each data.items as item (item.id)}
      <tr><td><a href="/account/orders/{item.id}" class="link link-hover font-medium">{item.name}</a></td><td class="hidden sm:table-cell">${Number(item.total).toFixed(2)}</td><td class="hidden md:table-cell"><span class="badge badge-sm">{statusLabels[item.status] ?? item.status}</span></td><td>{item.order_date ?? "—"}</td>
