...
<svelte:head><title>{item.name} - Cotizaciones</title></svelte:head>
<div class="mb-4"><a href="/account/quotes" class="link link-neutral text-sm">&larr; Volver</a></div>

{#if editing}
<div class="card bg-base-200 p-6 max-w-xl">
  <div class="flex justify-between items-center mb-4"><h2 class="text-lg font-semibold">Editar Cotización</h2><button class="btn btn-ghost btn-xs" onclick={() => editing = false}>✕</button></div>
  <form method="POST" action="?/update" use:enhance={handleSubmit}>
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      <div><label class="label" for="opportunity_id"><span class="label-text">Opportunity ID</span></label><input id="opportunity_id" name="opportunity_id" type="text" class="input input-bordered w-full" value={form?.opportunity_id ?? item.opportunity_id ?? ""} /></div>
      ...
      <div><label class="label" for="valid_until"><span class="label-text">Válido Hasta</span></label><input id="valid_until" name="valid_until" type="date" class="input input-bordered w-full" value={form?.valid_until ?? item.valid_until ?? ""} /></div>
...
<div class="card bg-base-200 p-6 max-w-xl">
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
    ...
    <div><span class="text-base-content/60 block text-xs uppercase tracking-wide">Válido Hasta</span><span>{item.valid_until ?? "—"}</span></div>
